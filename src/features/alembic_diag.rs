...
fn zero_range() -> types::Range {
    types::Range { start_line: 0, start_col: 0, end_line: 0, end_col: 0 }
}
...
fn w702_multiple_heads(
    ...
    let Some(ref revision) = mf.revision else { return };
...
fn h703_unknown_table(mf: &MigrationFile, state: &WorkspaceState, out: &mut Vec<Diagnostic>) {
    ...
    for op in &mf.op_calls {
        ...
        let Some(ref tref) = op.table_name else { continue };
...
fn w704_null_constraint_name(mf: &MigrationFile, out: &mut Vec<Diagnostic>) {
    for op in &mf.op_calls {
        let Some(r) = op.null_constraint_name_range else { continue };
        out.push(diag(
            "SQLA-W704",
            DiagnosticSeverity::WARNING,
            format!(
...
mod tests {
    ...
    fn def_range() -> MRange {
        MRange { start_line: 0, start_col: 0, end_line: 0, end_col: 10 }
    }

    fn dr(line: u32, start: u32, end: u32) -> MRange {
        MRange { start_line: line, start_col: start, end_line: line, end_col: end }
    }
    ...
    fn op_call(operation: &str, table: Option<&str>) -> OpCall {
        OpCall {
            ...
            full_range: def_range(),
            table_name: table.map(|t| TableRef { name: t.to_string(), range: def_range() }),
            column_name: None,
    ...
    fn req_alm_03_broken_chain_fires_w701() {
        ...
        state.revision_index.insert("a1".to_string(), uri("file:///a.py"));
    ...
    fn req_alm_03_merge_one_missing_fires_once() {
        let state = WorkspaceState::new();
        state.revision_index.insert("a1".to_string(), uri("file:///a.py"));
        let mf = mf_base("c3", DownRevision::Multiple(vec!["a1".to_string(), "b2".to_string()]));
        let heads = HashSet::new();
        let diags = check_migration(&mf, &state, &heads);
    ...
    fn req_alm_04_multiple_heads_fire_w702() {
        let state = WorkspaceState::new();
        state.revision_index.insert("h1".to_string(), uri("file:///h1.py"));
        state.revision_index.insert("h2".to_string(), uri("file:///h2.py"));
...
"#;
        ...
        let tree = parser.parse(src, None).unwrap();
        let mf = extract_migration(src, &tree).unwrap();
        let op = mf.op_calls.iter().find(|o| o.operation == "add_column").unwrap();
        assert_eq!(op.table_name.as_ref().map(|t| t.name.as_str()), Some("posts"));
        assert_eq!(op.column_name.as_ref().map(|c| c.name.as_str()), Some("title"));
    }
...
"#;
        ...
        let tree = parser.parse(src, None).unwrap();
        let mf = extract_migration(src, &tree).unwrap();
        let op = mf.op_calls.iter().find(|o| o.operation == "drop_constraint").unwrap();
        assert!(op.null_constraint_name_range.is_some(), "None constraint should be detected");
    }
...
"#;
        ...
        let mf = extract_migration(src, &tree).unwrap();
        let op = mf.op_calls.iter().find(|o| o.operation == "drop_constraint").unwrap();
        assert!(op.null_constraint_name_range.is_none(), "named constraint should not be flagged");
    }
