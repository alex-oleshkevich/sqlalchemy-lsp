...
const OP_SIGNATURES: &[SigInfo] = &[
    SigInfo { label: "op.add_column(table_name, column, *, schema=None)", params: &[("table_name", "Table to add the column to."), ("column", "sa.Column() object to add."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.drop_column(table_name, column_name, *, schema=None)", params: &[("table_name", "Table to remove the column from."), ("column_name", "Name of the column to remove."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.alter_column(table_name, column_name, *, nullable=None, new_column_name=None, type_=None, existing_type=None)", params: &[("table_name", "Table containing the column."), ("column_name", "Column to alter."), ("nullable=None", "New nullable setting."), ("new_column_name=None", "Rename the column."), ("type_=None", "New type for the column."), ("existing_type=None", "Current type (required by some DBs).")] },
    SigInfo { label: "op.create_table(table_name, *columns, **kw)", params: &[("table_name", "Name of the new table."), ("*columns", "sa.Column() objects."), ("**kw", "Additional keyword arguments.")] },
    SigInfo { label: "op.drop_table(table_name, *, schema=None)", params: &[("table_name", "Table to drop."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.create_index(index_name, table_name, columns, *, unique=False, schema=None)", params: &[("index_name", "Index name."), ("table_name", "Table to index."), ("columns", "List of column names or expressions."), ("unique=False", "Create a unique index."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.drop_index(index_name, table_name=None, *, schema=None)", params: &[("index_name", "Index to drop."), ("table_name=None", "Table (required for some dialects)."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.create_unique_constraint(constraint_name, table_name, columns, *, schema=None)", params: &[("constraint_name", "Constraint name."), ("table_name", "Table to constrain."), ("columns", "Columns to include."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.drop_constraint(constraint_name, table_name, *, type_=None, schema=None)", params: &[("constraint_name", "Constraint to drop."), ("table_name", "Table holding the constraint."), ("type_=None", "Constraint type hint."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.create_foreign_key(constraint_name, source_table, referent_table, local_cols, remote_cols)", params: &[("constraint_name", "Constraint name."), ("source_table", "Table holding the FK."), ("referent_table", "Table being referenced."), ("local_cols", "Local column names."), ("remote_cols", "Referenced column names.")] },
    SigInfo { label: "op.create_check_constraint(constraint_name, table_name, condition, *, schema=None)", params: &[("constraint_name", "Constraint name."), ("table_name", "Table to constrain."), ("condition", "SQL condition expression."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.rename_table(old_table_name, new_table_name, *, schema=None)", params: &[("old_table_name", "Current table name."), ("new_table_name", "New table name."), ("schema=None", "Optional schema name.")] },
    SigInfo { label: "op.execute(sqltext, *, execution_options=immutabledict({}))", params: &[("sqltext", "SQL string or ClauseElement to execute."), ("execution_options=immutabledict({})", "Execution options.")] },
    SigInfo { label: "op.bulk_insert(table, rows, *, multiinsert=True)", params: &[("table", "Table object to insert into."), ("rows", "List of dicts to insert."), ("multiinsert=True", "Use multi-row INSERT syntax.")] },
];
...
const REL_SIG: SigInfo = SigInfo {
    ...
    params: &[
        ...
        ("back_populates=", "Attribute name on the target that mirrors this relationship."),
        ("lazy=", "Loading strategy: select, joined, subquery, selectin, raise, dynamic, noload."),
        ("uselist=", "True for collection, False for scalar (defaults based on cardinality)."),
        ("secondary=", "Association table for many-to-many relationships."),
        ("cascade=", "Cascade rules: save-update, merge, delete, delete-orphan, all."),
        ("order_by=", "Ordering applied to the collection."),
        ("foreign_keys=", "Explicit FK columns when multiple FKs ambiguate the join."),
...
const MC_SIG: SigInfo = SigInfo {
    ...
    params: &[
        ("type_=", "Column type (e.g. String, Integer). Often inferred from Mapped[…]."),
        ("primary_key=", "True to mark as primary key (default: False)."),
        ("nullable=", "Allow NULL values (default: True unless primary_key)."),
        ...
        ("name=", "Explicit DB column name when different from the attribute name."),
...
fn innermost_call(prefix: &str) -> Option<(String, usize)> {
    ...
    let start = trimmed.rfind(|c: char| !(c.is_alphanumeric() || c == '_' || c == '.')).map(|i| i + 1).unwrap_or(0);
...
fn kwarg_key_at_cursor(prefix: &str, open: usize) -> Option<String> {
    ...
    let after_last_comma = inner.rfind([',', '('])
        .map(|i| &inner[i + 1..])
        .unwrap_or(inner);
    let trimmed = after_last_comma.trim_start();
    if let Some(eq_pos) = trimmed.find('=') {
        let key: String = trimmed[..eq_pos].chars()
...
fn synthesize_constructor(
    ...
    let param_labels: Vec<String> = cols.iter().map(|c| col_param_label(&c.name, &c.mapped_type, &c.args)).collect();
    let label = format!("{}({})", model_name, param_labels.join(", "));

    let parameters: Vec<ParameterInformation> = cols.iter()
        .map(|c| ParameterInformation {
            label: ParameterLabel::Simple(col_param_label(&c.name, &c.mapped_type, &c.args)),
            documentation: Some(tower_lsp_server::ls_types::Documentation::String(col_param_doc(&c.args).into())),
        })
        .collect();
    ...
    let active = if let Some(key) = kwarg_key_at_cursor(prefix, open) {
        cols.iter().position(|c| c.name == key).map(|i| i as u32).unwrap_or(positional)
    } else {
    ...
    Some(SignatureHelp {
        signatures: vec![SignatureInformation {
            ...
            documentation: Some(tower_lsp_server::ls_types::Documentation::String(
                format!("Synthesized from {model_name}'s mapped columns."),
            )),
...
fn static_sig(sig: &SigInfo, active: u32) -> SignatureHelp {
    ...
    let parameters = sig.params.iter().map(|(label, doc)| ParameterInformation {
        label: ParameterLabel::Simple(label.to_string()),
        documentation: Some(tower_lsp_server::ls_types::Documentation::String(doc.to_string())),
    }).collect();
    ...
    match base_name.as_str() {
        ...
        "relationship" => {
            ...
            let active_kw = if let Some(key) = kwarg_key_at_cursor(prefix, open) {
                REL_SIG.params.iter().position(|(p, _)| p.starts_with(&key))
                    .map(|i| i as u32).unwrap_or(active)
            } else {
                active
            };
        ...
        name if source.contains("op.") => {
            ...
            if let Some(op_name) = full_name.strip_prefix("op.") {
                let sig = OP_SIGNATURES.iter().find(|s| s.label.starts_with(&format!("op.{op_name}(")))?;
...
mod tests {
    ...
    fn uri(s: &str) -> tower_lsp_server::ls_types::Uri { s.parse().unwrap() }
    ...
    fn col(name: &str, mt: MappedType, nullable: bool, rng: Range) -> Column {
        Column {
            name: name.to_string(), key: None, mapped_type: mt,
            args: ColumnArgs { nullable, primary_key: false, unique: false, index: false, default: None, server_default: None },
            foreign_key: None, doc: None, name_range: rng, full_range: rng,
        }
    }
    ...
    fn req_sig_07_08_constructor_synthesized() {
        ...
        let mut model = crate::model::types::Model {
            name: "User".to_string(), table_name: Some("users".to_string()), bases: vec![],
            columns: HashMap::new(), relationships: HashMap::new(), table_args: vec![],
            duplicate_columns: vec![], docstring: None, name_range: rng(0, 6, 0, 10), full_range: rng(0, 0, 20, 0),
        };
        ...
        assert!(label.contains("Optional"), "nullable bio should be optional: {label}");
    }
    ...
    fn req_sig_09_kwarg_active_param() {
        ...
        assert_eq!(help.active_parameter, Some(1), "email kwarg should highlight param 1");
    }
    ...
    fn req_sig_10_op_add_column() {
        ...
        let help = provide_signature_help(&u, &src_with_op, pos(1, (src.len()) as u32), &state).unwrap();
        assert!(help.signatures[0].label.contains("op.add_column(table_name, column"));
