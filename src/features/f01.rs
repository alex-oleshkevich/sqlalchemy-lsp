...
fn base_type(t: &MappedType) -> &MappedType {
    if let MappedType::Optional(inner) = t {
        base_type(inner)
    } else {
        t
    }
...
fn w101_missing_tablename(model: &Model, out: &mut Vec<Diagnostic>) {
    ...
    let is_abstract = model
        .bases
        .iter()
        .any(|b| SA_ABSTRACT_BASES.contains(&b.as_str()));
    if !is_abstract {
        out.push(d(
            "SQLA-W101",
...
fn e102_duplicate_tablename(
    ...
    let Some(ref table) = model.table_name else {
        return;
    };
    let Some(owners) = table_owners.get(table) else {
        return;
    };
    if owners.len() <= 1 {
        return;
    ...
    if let Some(other) = owners.iter().find(|n| *n != &model.name) {
        out.push(d(
            ...
            format!(
                "Duplicate table name `{}` (also used by `{}`)",
                table, other
            ),
            model.name_range,
        ));
...
fn e105_table_arg_column_not_found(model: &Model, out: &mut Vec<Diagnostic>) {
    for table_arg in &model.table_args {
        for (col_name, range) in table_arg.columns.iter().zip(table_arg.column_ranges.iter()) {
...
fn e301_unknown_fk_table(model: &Model, state: &WorkspaceState, out: &mut Vec<Diagnostic>) {
    for col in model.columns.values() {
        let Some(ref fk) = col.foreign_key else {
            continue;
        };
        if !state.table_index.contains_key(&fk.table) && !state.model_index.contains_key(&fk.table)
        {
            out.push(d(
                "SQLA-E301",
                DiagnosticSeverity::ERROR,
...
fn e302_fk_column_not_found(model: &Model, state: &WorkspaceState, out: &mut Vec<Diagnostic>) {
    for col in model.columns.values() {
        ...
        let Some(target_loc) = state.model_index.get(&target_name) else {
            continue;
        };
...
fn w303_fk_type_mismatch(model: &Model, state: &WorkspaceState, out: &mut Vec<Diagnostic>) {
    for col in model.columns.values() {
        ...
        let Some(target_col) = target_model.columns.get(&fk.column) else {
            continue;
        };
        if let Some(false) = types_match(&col.mapped_type, &target_col.mapped_type) {
            out.push(d(
                ...
                format!(
                    ...
                    col.name, col.mapped_type, target_name, fk.column, target_col.mapped_type,
                ),
                fk.range,
            ));
        }
...
fn h406_missing_fk_for_relationship(
    ...
    for rel in model.relationships.values() {
        ...
        let has_fk = model.columns.values().any(|col| {
            ...
            fk.table == rel.target_model || target_table.as_deref().is_some_and(|t| t == fk.table)
        });
        if has_fk {
            continue;
        }
        ...
        let naming_present = model.columns.contains_key(&format!("{}_id", rel.name))
            || model
                .columns
...
fn target_has_fk_to(state: &WorkspaceState, target_model_name: &str, current: &Model) -> bool {
    let Some(loc) = state.model_index.get(target_model_name) else {
        return false;
    };
...
fn w408_unknown_cascade(model: &Model, out: &mut Vec<Diagnostic>) {
    for rel in model.relationships.values() {
        let Some(ref cascade) = rel.cascade else {
            continue;
        };
                        ...
                        if cycle_len > 2 {
                            ...
                            for i in start_idx..path.len() {
                                ...
                                if let Some(edges_from) = adj.get(from.as_str()) {
                                    if let Some((_, r)) = edges_from.iter().find(|(t, _)| t == to) {
                                        cycles.push(*r);
                                    }
                                }
                            }
...
mod tests {
    ...
    fn fk_col(name: &str, table: &str, col: &str, nullable: bool) -> Column {
        Column {
            name: name.to_string(),
            key: None,
            mapped_type: if nullable {
                MappedType::Optional(Box::new(MappedType::Int))
            } else {
                MappedType::Int
            },
            args: ColumnArgs {
    ...
    fn req_diag_03_duplicate_column() {
        ...
        model
            .duplicate_columns
            .push(("title".to_string(), def_range()));
        let diags = check_file(&[model], &state);
    ...
    fn req_diag_06_unknown_fk_table() {
        ...
        model.columns.insert(
            "author_id".to_string(),
            fk_col("author_id", "user", "id", false),
        );
        let diags = check_file(&[model], &state);
    ...
    fn req_diag_06_fk_no_false_positive_when_target_indexed() {
        ...
        post.columns.insert(
            "author_id".to_string(),
            fk_col("author_id", "users", "id", false),
        );
    ...
    fn req_diag_09_rel_target_not_found() {
        ...
        model
            .relationships
            .insert("author".to_string(), rel("author", "Ghost", false, None));
        let diags = check_file(&[model], &state);
    ...
    fn req_diag_10_back_populates_mismatch() {
        ...
        post.relationships.insert(
            "author".to_string(),
            rel("author", "User", false, Some("posts")),
        );

        let mut user = bare_model("User", Some("users"));
        user.relationships.insert(
            "posts".to_string(),
            rel("posts", "Post", true, Some("wrong")),
        );
    ...
    fn req_diag_18_circular_relationship_three_models() {
        ...
        a.relationships
            .insert("b".to_string(), rel("b", "B", false, None));
        let mut b = bare_model("B", Some("b_t"));
        b.relationships
            .insert("c".to_string(), rel("c", "C", false, None));
        let mut c = bare_model("C", Some("c_t"));
        c.relationships
            .insert("a".to_string(), rel("a", "A", false, None));
    ...
    fn req_diag_18_back_populates_pair_silent() {
        ...
        user.relationships.insert(
            "posts".to_string(),
            rel("posts", "Post", true, Some("author")),
        );
        let mut post = bare_model("Post", Some("posts"));
        post.relationships.insert(
            "author".to_string(),
