...
- **Three diagnostics** — `SQLA-W701` broken-migration-chain, `SQLA-W702` multiple-heads, and `SQLA-H703` unknown-migration-table.
...
The catalog of the three rules, for quick scanning:
...
| E2E-05 | Fix the broken chain (rename `down_revision` back), save | happy | On re-index, the `SQLA-W701` diagnostic clears without reopening. |
| E2E-06 | Request completion after `op.` in a migration | happy | The `op.*` operations are offered; the same position in a non-migration file offers none. |
| E2E-07 | Request completion at an `op.*` table argument | happy | Indexed table names are offered; after a table, that table's columns are offered. |
| E2E-08 | Request signature help inside an `op.add_column(…)` call | happy | A `SignatureHelp` returns with the active parameter highlighted. |
| E2E-09 | Go-to-definition on an `op.*` table string | happy | Resolves to the owning model's class range in its file. |
| E2E-10 | Go-to-definition on an `op.*` column string | happy | Resolves to the column's attribute range; an unresolved column returns null. |
...
| AC-05 | A broken chain on disk | the user restores the `down_revision` and saves | the `SQLA-W701` diagnostic clears on the next re-index. |
| AC-06 | The cursor after `op.` in a migration | the user requests completion | the Alembic operations are offered; a plain module offers none. |
| AC-07 | The cursor on an `op.*` table reference | the user invokes go-to-definition | the editor navigates to the owning model's class. |
...
- **Decision** — The three `7xx` rules ship as `SQLA-W701`/`W702`/`H703`, ported from the legacy server's `broken-migration-chain`/`multiple-heads`/`unknown-migration-table` with the constitution's `SQLA-` namespace. The unknown-table rule stays a *hint* because migrating model-less tables is legitimate (P4).
