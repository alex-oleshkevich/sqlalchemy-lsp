...
/// REQ-REL-08: .SRCINFO template is committed (not generated by makepkg, which is Arch-only).
#[test]
fn aur_srcinfo_template_exists_without_makepkg() {
    ...
    assert!(
        !release_yml.contains("makepkg"),
        "publish-aur must not require makepkg (Arch-only tool); use sed to update .SRCINFO template"
    );
}
